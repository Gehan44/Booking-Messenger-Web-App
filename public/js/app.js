...
const savePNGButton = wrapper.querySelector("[data-action=save]");
...
savePNGButton.addEventListener("click", async () => {
    const editTerm = document.getElementById('editTerm').value;
    if (signaturePad.isEmpty()) {
        alert("Please provide a signature first.");
    } else {
        const dataURL = signaturePad.toDataURL();
        try {
            const response = await fetch('/sign/store', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
                body: JSON.stringify({ signature: dataURL, editTerm: editTerm })
            });
            if (response.ok) {
            ...
            console.error(error);
            alert("An error occurred while storing the signature.");
        }
    }
});
